//Nessa biblioteca, temos a implementação de três estrutras de dados paralelas, que são utilizadas em conjunto em momentos diversos.
typedef struct
{
    //Estrutura geral para armazenar uma cadeia de caracteres qualquer.
    char *value;
}Info;

typedef struct BST
{
    //Nó de uma arvore de busca binária que não implementa estrutura específica pra cabeça.
	int balance;
    Info *inf;
...
typedef struct Node
{
    //Nó de uma lista com elementos de informação Info. Implementa Head como cabeça da estrutura.
    Info *inf;
    struct Node *next;
}Node;
...
typedef struct
{
    //É a cabeça de uma lista cujos elementos são estruturas Node.
...
BSTHead *createBSTHeadLST()
{//essa função sera chamada exclusivamente pela função insertLastBSTLST(...)
    BSTHead *ptr = NULL;
...
void insertLastBSTLST(BSTHead **lst,BST *inf)
{//cria a cabeça automaticamente caso seja passado um valor NULL
    BSTNode *ptr = NULL;
    if(!inf)
        return;
    if(!*lst)
...
//prototipadas aqui para preservar a ordem da implementação
void destroyBST(BST *bst);
BST *createNodeBST(Info *inf);
int isEmptyBST(BST *bst);

Head *createHeadLST()
{//essa função sera chamada exclusivamente pela função insertLastLST(...)
    Head *ptr = NULL;
...
void insertLastLST(Head **lst, BST *bst)
{
    ...
    if(*lst && ptr)
    {
        ...
        {//do contrário apenas last aponta pra ptr
            (*lst)->last->next = ptr;
            (*lst)->last = ptr;
        }
    }
...
void freeLST(Head *lst)
{
    ...
    if(lst)
    {
        ptr = lst->first;
        while(ptr)
        {//libera memória de toda info e todo nó em Head 
            pptr = ptr;
            ptr = ptr->next;
            free(pptr->inf);
            free(pptr);
...
{//retorna o elemento de informação do nó bst
    Info *ptr = NULL;
    if(bst)
        ptr = bst->inf;
    return ptr;
...
{//retorna qual a altura da arvore
...
int weightBST(BST *bst)
{//retorna quantos nos existem na arvore
	if(isEmptyBST(bst))
		return 0;
...
int auxInsertAVL(BST **avl, BST *new, int *growup)
{
	...
	if(strcmp(rootBST(new)->value, rootBST(*avl)->value) == 0)//repetido
		return 0;
	if(strcmp(rootBST(new)->value, rootBST(*avl)->value) < 0)
    {//caminhar pela arvore esquerda
...
BST *deleteBST(BST *bst, Info *inf)
{
    ...
    {//encontrou o elemento, vamos deletar ele: 
        if(isEmptyBST(rightBST(bst)))
        {//se não tem elemento à direita, retorne o elemento à esquerda
            temp = leftBST(bst);
            free(bst->inf);
            free(bst);
        }
        else if(isEmptyBST(leftBST(bst)))
        {//se nao tem elemento à esquerda, retorne o elemento à direita
            temp = rightBST(bst);
            free(bst->inf);
            free(bst);
        }
        else
        {//agora caso tenha os dois lados: 
...
int deleteAVL(BST **bst, char *value)
{
    ...
    if(isEmptyBST(*bst))
        return 0;
    if(strcmp(value, rootBST(*bst)->value) < 0)
    {//caso esteja pela arvore esquerda:
    ...
    if(strcmp(rootBST(*bst)->value, value) < 0)
    {//caso esteja pela arvore direita:
    ...
    {//encontrou o elemento
        if(isEmptyBST(leftBST(*bst)) || isEmptyBST(rightBST(*bst)))
        {// nó tem uma subarvore ou nenhuma
            temp = *bst;
            if(leftBST(*bst) != NULL)
                *bst = leftBST(*bst);
            else
...
Head *takeWhileLST(Head *lst, Info *inf)
{
    ...
    Head *newlst = NULL;
    BST *temp = NULL;
    for(ptr = lst->first; ptr && ptr->inf->value != inf->value; ptr = ptr->next)
    {//pra cara informação antes de inf(root), insira ela na nova lista
...
Head *dropWhileLST(Head *lst, Info *inf)
{
    ...
    if(ptr)//encontrado inf(root), caso ele seja diferente de NULL faremos
        ...
        {//para cada Node depois de inf(root)
